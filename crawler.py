...
def getResult(a):
    a=a.lower()
    req = requests.get("https://tioj.ck.tp.edu.tw/users/"+a.lower(), allow_redirects=False)
...
def getPage():
    eprint("Getting Page")
    req = requests.get("https://tioj.ck.tp.edu.tw/users", allow_redirects=False)
    sp = BeautifulSoup(req.text, "html.parser")
...
def getNext(a):
    ...
    for i in range(1,getPage()+1):
        ...
        req = requests.get(f"https://tioj.ck.tp.edu.tw/users?page={i}")
        ...
        if found:
            ...
            if nxt:
                ...
                if i==1:
                    ...
                    temp = BeautifulSoup( requests.get("httsp://tioj.ck.tp.edu.tw/users?page="+str(i-1)).text, "html.parser").findAll("a", href=re.compile(r'/users/[^\?]'))
